...
interface Props {
  onClose: () => void
  onAdd: (name: string, schedule: string) => Promise<void>
}
...
export default function AddClassModal({ onClose, onAdd }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!name.trim()) return
    setLoading(true)
    await onAdd(name.trim(), schedule.trim())
    setLoading(false)
  }
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative bg-white rounded-t-2xl sm:rounded-2xl shadow-xl w-full sm:max-w-[400px] sm:mx-4 p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-[18px] font-bold text-[#191F28]">반 추가</h2>
          <button
            type="button"
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-full text-[#6B7684] hover:bg-[#F2F4F6] transition-colors text-[18px]"
          >
            ✕
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[13px] font-medium text-[#6B7684]">반 이름</label>
            <input
              type="text"
              value={name}
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[13px] font-medium text-[#6B7684]">
              수업 요일/시간 <span className="text-[#ADB5BD] font-normal">(선택)</span>
            </label>
            <input
              type="text"
              value={schedule}
              onChange={e => setSchedule(e.target.value)}
              placeholder="예: 월/수 19:00~20:30"
              className="w-full h-[52px] px-4 rounded-xl border border-[#E5E8EB] bg-[#F9FAFB] text-[15px] text-[#191F28] placeholder:text-[#ADB5BD] outline-none transition-all focus:border-[#3182F6] focus:bg-white focus:ring-2 focus:ring-[#3182F6]/10"
            />
          </div>
